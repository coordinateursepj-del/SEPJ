 ...
 *   getenv()  →  this file  →  Admin ▸ Settings ▸ Email Configuration (DB)
 *   →  hard-coded defaults inside get_mail_config()
 *
 * mail.local.php never reaches production via the GitHub → OVH deploy
 * (it's .gitignore'd), so if it's not manually placed on the server,
 * get_mail_config() falls back to the SMTP host/port/secure/username/
 * password stored in site_settings, editable from Admin ▸ Settings.
 */
