 ...
 *  3. site_settings DB rows      — admin-panel overrides for non-sensitive keys
 *  4. Hard-coded defaults below
 *
 * Credentials (username, password) come ONLY from the PHP config files,
 * never from the DB, to reduce the blast radius of a database compromise.
 */
function get_mail_config(): array
{
    ...
    // DB overrides for non-sensitive display settings only
    // Credentials stay in PHP files — not pulled from DB
    try {
        $rows = db()->query("
            SELECT setting_key, value_raw FROM site_settings
            WHERE setting_key IN ('smtp_from_email','smtp_from_name')
