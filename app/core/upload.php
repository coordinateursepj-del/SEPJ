...
function upload_file(array $file, string $subdirectory = 'general'): array
{
    ...
    // Ensure .htaccess in uploads directory and subdirectories
    $uploadRootDir = UPLOAD_PATH;
    $htaccessDirs = [$uploadRootDir, $uploadDir];
    foreach ($htaccessDirs as $dir) {
        $htaccessFile = $dir . '/.htaccess';
        if (!file_exists($htaccessFile)) {
            $htaccessContent = "# SEPJ Gabès - Upload Security\n# Allow image files, block PHP execution and directory listing\n\n# Disable directory listing\nOptions -Indexes\n\n# Block PHP execution\n<FilesMatch \"\\.(php|php\\d+|phtml|phar|shtml|cgi|pl|py|asp|aspx|exe|inc)$\">\n    Order Allow,Deny\n    Deny from all\n</FilesMatch>\n\n# Allow image access\n<FilesMatch \"\\.(jpg|jpeg|png|webp|gif|svg|ico)$\">\n    Order Allow,Deny\n    Allow from all\n</FilesMatch>\n\n# Default: Allow access to everything else (images, css, etc)\n<IfModule mod_authz_core.c>\n    Require all granted\n</IfModule>\n";
            if (file_put_contents($htaccessFile, $htaccessContent)) {
                chmod($htaccessFile, 0644);
                error_log("INFO: Created .htaccess security file - {$htaccessFile}");
            } else {
                error_log("WARNING: Failed to create .htaccess file - {$htaccessFile}");
            }
        }
    }
