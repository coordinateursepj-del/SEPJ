...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            $stmt = db()->prepare("
                INSERT INTO content_items (type, rse_category, slug, title_ar, title_fr, title_en, summary_ar, summary_fr, summary_en, body_ar, body_fr, body_en, featured_image, status, is_featured, published_at, created_by, created_at, updated_at)
                VALUES (:type, :rse_category, :slug, :title_ar, :title_fr, :title_en, :summary_ar, :summary_fr, :summary_en, :body_ar, :body_fr, :body_en, :featured_image, :status, :is_featured, :published_at, :created_by, NOW(), NOW())
            ");
            $stmt->execute([
                'type' => $type,
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="glass-card-static p-4">
                        <label class="block text-sm font-medium text-emerald-200 mb-2">
                            <?php if ($lang === 'ar'): ?>الصورة الرئيسية
                            <?php elseif ($lang === 'fr'): ?>Image à la une
                            <?php else: ?>Featured Image
                            <?php endif; ?>
                        </label>
                        ...
                        <div class="mt-3">
                            <img id="imagePreview" class="hidden w-48 h-32 object-cover rounded-lg border border-white/10">
                        </div>
                    </div>
