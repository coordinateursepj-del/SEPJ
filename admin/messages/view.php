<?php require_once dirname(__DIR__,2).'/app/config/app.php'; require_once ROOT_PATH.'/app/core/db.php'; require_once ROOT_PATH.'/app/core/auth.php'; require_once ROOT_PATH.'/app/core/csrf.php'; require_once ROOT_PATH.'/app/core/helpers.php'; session_start_secure(); require_login(); $id=(int)($_GET['id']??0); $lang=current_lang(); $stmt=db()->prepare("SELECT * FROM contact_messages WHERE id=:id"); $stmt->execute(['id'=>$id]); $msg=$stmt->fetch(); if(!$msg){redirect('index.php');} // Mark as read automatically if($msg['status']==='new'){db()->prepare("UPDATE contact_messages SET status='read' WHERE id=:id")->execute(['id'=>$id]);} ?>
