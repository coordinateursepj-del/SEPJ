...
<main class="flex-1 overflow-y-auto p-6"><h1 class="text-2xl font-bold text-white mb-6">✉️ <?=$lang==='ar'?'الرسائل':($lang==='fr'?'Messages':'Messages')?></h1>
<div class="flex gap-2 mb-4"><?php foreach([''=>($lang==='ar'?'الكل':($lang==='fr'?'Tous':'All')),'new'=>($lang==='ar'?'جديد':($lang==='fr'?'Nouveau':'New')),'read'=>($lang==='ar'?'مقروء':($lang==='fr'?'Lu':'Read')),'archived'=>($lang==='ar'?'مؤرشف':($lang==='fr'?'Archivé':'Archived'))] as $k=>$l):?><a href="?<?=$k?'status='.$k:''?>" class="glass-btn text-sm <?=!$status&&!$k||$status===$k?'glass-btn-primary':''?>"><?=e($l)?></a><?php endforeach;?></div>
...
<div class="min-w-0 flex-1"><p class="text-white font-medium"><?=e($msg['name'])?><?php if($msg['status']==='new'):?><span class="ml-2 text-xs bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full"><?=$lang==='ar'?'جديد':($lang==='fr'?'Nouveau':'New')?></span><?php endif;?></p>
