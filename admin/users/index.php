...
echo e($rl);
?></td><td class="p-3 text-center"><?=status_badge($u['status'], $lang)?></td>
...
<div class="relative"><input type="password" name="password" class="form-input w-full pr-12" placeholder="<?=$editUser?($lang==='ar'?'كلمة مرور جديدة (اختياري)':($lang==='fr'?'Nouveau mot de passe (optionnel)':'New password (optional)')):($lang==='ar'?'كلمة المرور':($lang==='fr'?'Mot de passe':'Password'))?>"><button type="button" onclick="togglePassword(this)" class="absolute inset-y-0 right-0 flex items-center pr-3 text-white/40 hover:text-emerald-300 transition-colors" tabindex="-1" aria-label="Toggle password visibility"><svg class="pw-eye w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg><svg class="pw-eye-off w-5 h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878l4.242 4.242"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-5.125-5.125M16.626 12.376A10.05 10.05 0 0112 7c-1.126 0-2.207.186-3.213.527"/></svg></button></div>
<div class="grid grid-cols-2 gap-2"><select name="role" class="form-input"><option value="editor"<?=$editUser&&$editUser['role']==='editor'?' selected':''?>><?=$lang==='ar'?'محرر':($lang==='fr'?'Éditeur':'Editor')?></option><option value="admin"<?=$editUser&&$editUser['role']==='admin'?' selected':''?>><?=$lang==='ar'?'مدير':($lang==='fr'?'Admin':'Admin')?></option></select>
